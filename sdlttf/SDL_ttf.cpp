...
TTF_Font* TTF_OpenFontIndexRW( SDL_RWops *src, int freesrc, int ptsize, long index )
{
	...
	auto pixelsize = ptsize;
	font->font_size_family = pixelsize;
    ...
    font->outline = 0;
	font->line_cap = FT_STROKER_LINECAP_BUTT;
	font->line_join = FT_STROKER_LINEJOIN_MITER;
	font->miter_limit = 10;
...
void _RasterCallback(const int y, const int count, const FT_Span* const spans, void* const user)
{
	std::vector<Span_> *sptr = (std::vector<Span_> *)user;
	for (int i = 0; i < count; ++i)
		sptr->push_back({ spans[i].x, y, spans[i].len, spans[i].coverage });
}
struct Rect_ {
...
static FT_Error Load_Glyph( TTF_Font* font, Uint16 ch, c_glyph* cached, int want )
{
    ...
    if ( ((want & CACHED_BITMAP) && !(cached->stored & CACHED_BITMAP)) ||
        ...
        if ( (font->outline > 0) && glyph->format != FT_GLYPH_FORMAT_BITMAP ) {
            ...
            FT_Stroker_Set( stroker, font->outline * 64, font->line_cap, font->line_join, font->miter_limit );
			...
			FT_BitmapGlyph xxx = (FT_BitmapGlyph)bitmap_glyph;
            if ( error ) {
			...
			outline = &((FT_OutlineGlyph)bitmap_glyph)->outline;
			std::vector<Span_> spans;
			...
			long width, rows;
			bitmap = (FT_Bitmap*)malloc(sizeof(FT_Bitmap));
			memset(bitmap, 0, sizeof(FT_Bitmap));
			if (!spans.empty()) {
				Rect_ rect = { spans.front().x, spans.front().y, spans.front().x, spans.front().y };				
				for (auto& span : spans) {
				...
				width = rect.width();
				rows = rect.height();
				unsigned char *buffer = (unsigned char*)malloc(width * rows);
				memset(buffer, 0, width * rows);
				for (auto& span : spans) {
					for (int w = 0; w < span.width; ++w)
						buffer[(int)((rows - 1 - (span.y - rect.ymin)) * width + span.x - rect.xmin + w)] = span.coverage;						
				}
				bitmap->buffer = buffer;
				bitmap->width = (int)width;
				bitmap->rows = (int)rows;
...
SDL_Surface *TTF_RenderUTF8_Blended_PremultipliedAlpha(TTF_Font *font, const char *text)
{
	SDL_bool first;
	int xstart;
	int width, height;
	...
	first = SDL_TRUE;
	xstart = 0;
	SDL_FillRect(textbuf, NULL, 0); /* Initialize with fg and 0 alpha */
	while (textlen > 0) {
		Uint16 c = UTF8_getch(&text, &textlen);
		if (c == UNICODE_BOM_NATIVE || c == UNICODE_BOM_SWAPPED) {
			continue;
		...
		for (row = 0; row < glyph->pixmap.rows; ++row) {
			...
			* */
			...
			for (col = width; col>0 && dst < dst_check; --col) {
				alpha = *src++;
				alpha = std::max<int>(alpha, *dst & 0xFF);
...
void TTF_SetFontOutline(TTF_Font* font, float outline)
{
    font->outline = outline;
    Flush_Cache( font );
}

void TTF_SetFontOutlineLineCap(TTF_Font * font, int line_cap)
{
	font->line_cap = (FT_Stroker_LineCap)line_cap;
	Flush_Cache(font);
}

void TTF_SetFontOutlineLineJoin(TTF_Font * font, int line_join)
{
	font->line_join = (FT_Stroker_LineJoin)line_join;
	Flush_Cache(font);
}

void TTF_SetFontOutlineMiterLimit(TTF_Font * font, int miter_limit)
{
	font->miter_limit = miter_limit << 16;
	Flush_Cache(font);
}

float TTF_GetFontOutline( const TTF_Font* font )
{
    return font->outline;
}
